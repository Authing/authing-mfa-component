...
export function SendCodeBtn(props: SendCodeProps) {
  const { t } = i18n

  const { sendDesc = t('mfa.clickSent'), beforeSend, btnRef, setSent, ...buttonProps } = props
  ...
  return (
    ...
    >
      ...
      {loading === false && (
        <span>{enabled ? sendDesc : t('mfa.retryAfterTime', { time: countDown })}</span>
      )}
    </Button>
  )
}
